...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (oauthError === "OAuthAccountNotLinked") {
      setError(
        "This account is already linked to another user. Please use a different account or sign out and try again."
      )
    }
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="max-w-lg w-full space-y-12">
          ...
          <div className="text-center space-y-6">
            ...
            <Link
              href="/about"
              className="text-sm text-black/50 hover:text-black underline inline-block transition-colors"
            >
              Learn more →
            </Link>
          </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-sm font-medium ${user.hasTwitter ? "text-green-600" : "text-gray-400"}`}
                  >
                    {user.hasTwitter
                      ? `✓ ${user.twitterHandle ? `@${user.twitterHandle}` : "Connected"}`
                      : "Not connected"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-sm font-medium ${user.hasGithub ? "text-green-600" : "text-gray-400"}`}
                  >
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-4">
                {!user?.hasTwitter && <ConnectButton provider="twitter" />}

                {user?.hasTwitter && !user?.hasGithub && <ConnectButton provider="github" />}
